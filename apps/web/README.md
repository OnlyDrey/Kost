...
```env
VITE_API_URL=/api   # Same-origin API path — baked at build time for the single-container image
```
...
- Create, edit, delete users with role assignment (Admin / Adult / Child role)
...
### Offline Support (PWA)
- Service worker (Workbox) caches hashed JS/CSS/image assets cache-first and navigation requests network-first
- Navigation always fetches a fresh `index.html` when online, preventing stale-shell issues on iOS Safari
- Mutations queued in IndexedDB when offline; drained automatically on reconnect
- Installable as a PWA on desktop and mobile (iOS: Add to Home Screen uses `apple-touch-icon.png`)


## Branding

- Header + sidebar use the shared `LogoMark` component (`src/components/Brand/LogoMark.tsx`).
- Public icon assets live in `public/` (`logo-mark.svg`, `favicon.svg`, `app-icon.svg`).
- Favicon and PWA icon metadata are configured in `index.html` and `public/manifest.webmanifest`.
