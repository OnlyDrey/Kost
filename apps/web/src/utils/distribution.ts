type DistributionMethod = "BY_INCOME" | "BY_PERCENT" | "FIXED" | "PERSONAL";
...
export function distributionFilterMatch(
  method: string,
  filterMethod: string,
  rules?: DistributionRulesLike,
): boolean {
  if (filterMethod === "ALL") return true;
  if (filterMethod === "FIXED_EQUAL")
    return (
      method === "FIXED" &&
      (!rules?.fixedRules || rules.fixedRules.length === 0)
    );
  if (filterMethod === "FIXED_AMOUNT")
    return (
      method === "FIXED" && !!rules?.fixedRules && rules.fixedRules.length > 0
    );
