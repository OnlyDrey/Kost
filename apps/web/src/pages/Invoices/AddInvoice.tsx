...
export default function AddInvoice() {
  ...
  // Default: exclude CHILD users from BY_INCOME selection
  useEffect(() => {
    if (users && users.length > 0 && !isEditing) {
      setIncomeUserIds(
        new Set(users.filter((u) => u.role !== "CHILD").map((u) => u.id)),
      );
    }
  }, [users, isEditing]);
  ...
  const totalPercent = Object.values(userPercents).reduce(
  ...
  const selectedForFixed = users
    ? users.filter((u) => incomeUserIds.has(u.id))
    : [];
  const equalPercent =
    selectedForFixed.length > 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (distributionMethod === "BY_PERCENT") {
      if (Math.abs(totalPercent - 100) > 0.01) {
        setError(t("invoice.percentsMustSum"));
        return;
      }
      ...
      if (percentRules.length === 0) {
        setError(t("invoice.specifyPercent"));
        return;
      }
    ...
    if (
      (distributionMethod === "BY_INCOME" || distributionMethod === "FIXED") &&
      incomeUserIds.size > 0
    ) {
      distributionRules = {
        ...(distributionRules ?? {}),
        userIds: Array.from(incomeUserIds),
      };
    }
    ...
    try {
      if (isEditing) {
        await updateInvoice.mutateAsync({
          id: id!,
          data: { ...sharedData, distributionRules },
        });
      } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {users.map((u) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isJunior && (
                        <span className="text-xs font-medium text-amber-500 dark:text-amber-400 flex-shrink-0">
                          {t("users.junior")}
                        </span>
                      )}
                      {checked && pct !== null ? (
          ...
          {distributionMethod === "BY_PERCENT" && users && users.length > 0 && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className={labelCls + " mb-0"}>
                  {t("invoice.percentPerUser")} *
                </label>
                <span
                  className={`text-sm font-medium ${Math.abs(totalPercent - 100) < 0.01 ? "text-green-500 dark:text-green-400" : "text-orange-500 dark:text-orange-400"}`}
                >
