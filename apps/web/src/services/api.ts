...
export const authApi = {
  loginWithPassword: (username: string, password: string, twoFactorCode?: string, recoveryCode?: string) =>
    api.post<{ message: string; user: User }>('/auth/login/password', { username, password, twoFactorCode, recoveryCode }),
  ...
  setupTwoFactor: () =>
    api.post<{ secret: string; otpauthUrl: string; recoveryCodes: string[] }>('/auth/2fa/setup'),
  ...
  regenerateRecoveryCodes: (currentPassword: string) =>
    api.post<{ recoveryCodes: string[] }>('/auth/2fa/recovery-codes/regenerate', { currentPassword }),
};
...
export const userApi = {
  ...
  create: (data: { username: string; name: string; role: 'ADMIN' | 'ADULT' | 'CHILD'; password?: string }) =>
    api.post<User>('/users', data),

  update: (id: string, data: { username?: string; name?: string; role?: 'ADMIN' | 'ADULT' | 'CHILD'; password?: string; avatarUrl?: string | null }) =>
...
export const paymentApi = {
  ...
  update: (id: string, data: { paidById?: string; amountCents?: number; note?: string; paidAt?: string }) =>
