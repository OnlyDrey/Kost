...
export class AuthController {
  ...
  @ApiResponse({ status: 501, description: 'Password authentication not enabled' })
  async register(
    @Body() dto: PasswordRegisterDto,
    @Res({ passthrough: true }) response: Response,
  ) {
    const result = await this.authService.registerWithPassword(dto.name, dto.email, dto.password);
  ...
  @ApiResponse({ status: 501, description: 'Magic link authentication not enabled' })
