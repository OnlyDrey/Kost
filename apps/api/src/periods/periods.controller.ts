...
export class PeriodsController {
  ...
  @ApiResponse({ status: 200, description: 'Period details with invoices and incomes' })
  ...
  @ApiResponse({ status: 200, description: 'Period status with payment summaries' })
  @ApiResponse({ status: 404, description: 'Period not found' })
  getPeriodStatus(@Param('id') id: string, @CurrentUser('familyId') familyId: string) {
  ...
  create(@Body() createPeriodDto: CreatePeriodDto, @CurrentUser('familyId') familyId: string) {
    return this.periodsService.create(createPeriodDto, familyId);
  }

  @Post(':id/close')
  @Roles(UserRole.ADMIN)
  @ApiOperation({ summary: 'Close a period and calculate settlements (Admin only)' })
  @ApiResponse({ status: 200, description: 'Period closed successfully with settlement data' })
  ...
  @ApiResponse({ status: 403, description: 'Forbidden - Admin only' })
  reopenPeriod(@Param('id') id: string, @CurrentUser('familyId') familyId: string) {
