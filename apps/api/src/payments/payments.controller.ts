...
export class PaymentsController {
  ...
  @ApiResponse({ status: 200, description: 'Payment summary showing who owes what' })
  getPeriodSummary(@Param('periodId') periodId: string, @CurrentUser('familyId') familyId: string) {
    return this.paymentsService.getPeriodPaymentSummary(periodId, familyId);
  }

  @Get('summary/user/:userId/period/:periodId')
  @ApiOperation({ summary: 'Get payment summary for a specific user in a period' })
  ...
  getUserSummary(
    ...
    return this.paymentsService.getUserPaymentSummary(userId, periodId, familyId);
  }
