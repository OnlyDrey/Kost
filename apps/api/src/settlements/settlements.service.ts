...
import {
  CreateSettlementEntryDto,
  ReverseSettlementEntryDto,
} from "./dto/create-settlement-entry.dto";
import { CreateSettlementPlanDto } from "./dto/create-settlement-plan.dto";
import { UpdateSettlementEntryDto } from "./dto/update-settlement-entry.dto";
...
export class SettlementsService {
  ...
  private parseSettlementData(raw: unknown): PeriodSettlementData {
    if (!raw || typeof raw !== "object")
      return { entries: [], plans: [], baseSettlements: [] };
    ...
    return {
      entries: Array.isArray(data.entries)
        ? (data.entries as SettlementEntry[])
        : [],
  ...
  private async loadPeriodSettlementData(
    periodId: string,
    familyId: string,
  ): Promise<PeriodSettlementData> {
    const period = await this.getPeriodOrThrow(periodId, familyId);
    return this.parseSettlementData(period.settlementData);
  }

  private async savePeriodSettlementData(
    periodId: string,
    familyId: string,
    data: PeriodSettlementData,
  ) {
    const period = await this.getPeriodOrThrow(periodId, familyId);
    const existing =
      period.settlementData && typeof period.settlementData === "object"
  ...
  private deriveBaseObligations(
    invoices: Array<{
      totalCents: number;
      shares: Array<{ userId: string; shareCents: number }>;
      payments: Array<{ paidById: string; amountCents: number }>;
    }>,
    ...
    for (const invoice of invoices) {
      ...
      for (const payment of invoice.payments) {
        for (const share of invoice.shares) {
          ...
          const shareOfPayment = Math.round(
            (share.shareCents / invoice.totalCents) * payment.amountCents,
          );
  ...
  async getPeriodSummary(periodId: string, familyId: string) {
    ...
    const baseMap = this.deriveBaseObligations(
      list[idx].period.invoices,
      list[idx].data,
    );
    const openingCredit = new Map<string, number>();
    const unresolvedWarnings: Array<{
      sourcePeriodId: string;
      fromUserId: string;
      toUserId: string;
      amountCents: number;
    ...
    for (let i = 0; i < idx; i += 1) {
      ...
      const currentBase = this.deriveBaseObligations(
        currentPeriod.period.invoices,
        currentPeriod.data,
      );
      const periodEntries = currentPeriod.data.entries.filter(
        (entry: any) => !entry.reversedAt,
      );
      ...
      for (const entry of periodEntries) {
        ...
        if (entry.type === "payment")
          paidByPair.set(key, (paidByPair.get(key) ?? 0) + entry.amountCents);
        if (entry.type === "write_down")
          writeDownByPair.set(
            key,
            (writeDownByPair.get(key) ?? 0) + entry.amountCents,
          );
        if (entry.type === "adjustment")
          adjustmentByPair.set(
            key,
            (adjustmentByPair.get(key) ?? 0) + entry.amountCents,
          );
      }
      ...
      for (const [key, base] of currentBase.entries()) {
        ...
        const planned = plans
          .filter(
            (plan: any) =>
              pairKey(plan.fromUserId, plan.toUserId) === key &&
              toMonthIndex(plan.startPeriodId) <=
                toMonthIndex(currentPeriod.period.id),
          )
          .reduce(
            (sum: number, plan: any) =>
              sum + this.applyPlanCharge(plan, currentPeriod.period.id, 0),
            0,
          );
        ...
        const effectiveDue = Math.max(
          base - appliedCredit + planned + adjustment - writeDown,
          0,
        );
        ...
        if (creditGenerated > 0) {
          openingCredit.set(
            key,
            (openingCredit.get(key) ?? 0) + creditGenerated,
          );
        } else {
          openingCredit.set(
            key,
            Math.max((openingCredit.get(key) ?? 0) - appliedCredit, 0),
          );
        }

        const hasPlan = plans.some(
          (plan: any) =>
            plan.sourcePeriodId === currentPeriod.period.id &&
            pairKey(plan.fromUserId, plan.toUserId) === key &&
            plan.status === "active",
        );
        if (
          remaining > 0 &&
          !hasPlan &&
          currentPeriod.period.status === PeriodStatus.CLOSED
        ) {
    ...
    const currentEntries = list[idx].data.entries.filter(
      (entry: any) => !entry.reversedAt,
    );
    ...
    for (const key of pairSet) {
      ...
      const plannedAddition = currentPlans
        ...
        .reduce(
          (sum: number, plan: any) =>
            sum + this.applyPlanCharge(plan, periodId, 0),
          0,
        );
      const payments = currentEntries
        .filter(
          (entry: any) =>
            entry.type === "payment" &&
            pairKey(entry.fromUserId, entry.toUserId) === key,
        )
      ...
      const writeDowns = currentEntries
        .filter(
          (entry: any) =>
            entry.type === "write_down" &&
            pairKey(entry.fromUserId, entry.toUserId) === key,
        )
        .reduce((sum: number, entry: any) => sum + entry.amountCents, 0);
      const effectiveDue = Math.max(
        base - credit + plannedAddition + adjustments - writeDowns,
        0,
      );
      ...
      summaryRows.push({
        ...
        unresolvedUnpaidWithoutPlanCents: unresolvedWarnings
          .filter(
            (warning) =>
              warning.fromUserId === fromUserId &&
              warning.toUserId === toUserId,
          )
    ...
    return {
      ...
      rows: summaryRows,
      totals: {
        totalOwedCents: summaryRows.reduce(
          (sum, row) => sum + row.effectiveDueCents,
          0,
        ),
        totalPaidCents: summaryRows.reduce(
          (sum, row) => sum + row.paymentsCents,
          0,
        ),
        totalCreditCents: summaryRows.reduce(
          (sum, row) => sum + row.carriedCreditCents + row.generatedCreditCents,
          0,
        ),
        totalUnpaidCents: summaryRows.reduce(
  ...
  async createEntry(
    ...
    if (dto.type === "payment" && period.status !== PeriodStatus.OPEN) {
      throw new BadRequestException(
        "Settlement payments can only be registered on open periods",
      );
    }
  ...
  async createPlan(
    ...
    const sourceSummary = await this.getPeriodSummary(
      dto.sourcePeriodId,
      familyId,
    );
    const sourcePair = sourceSummary.rows.find(
      (row) =>
        row.fromUserId === dto.fromUserId && row.toUserId === dto.toUserId,
    );
    const unresolved = sourcePair?.remainingCents ?? 0;
    if (unresolved <= 0)
      throw new BadRequestException(
        "No unresolved unpaid balance for selected pair",
      );

    const startPeriodId = dto.startPeriodId ?? sourceSummary.periodId;
    await this.getPeriodOrThrow(startPeriodId, familyId);

    const targetData = await this.loadPeriodSettlementData(
      dto.sourcePeriodId,
      familyId,
    );
    ...
    await this.savePeriodSettlementData(
      dto.sourcePeriodId,
      familyId,
      targetData,
    );
  ...
  async updateEntry(
    ...
    if (existing.reversedAt)
      throw new BadRequestException("Reversed entries cannot be edited");
    if (existing.type === "reversal")
      throw new BadRequestException("Reversal entries cannot be edited");
    if (existing.type === "payment" && period.status !== PeriodStatus.OPEN) {
      throw new BadRequestException(
        "Settlement payments can only be edited on open periods",
      );
    }
  ...
  async reverseEntry(
    periodId: string,
    entryId: string,
    dto: ReverseSettlementEntryDto,
    familyId: string,
    actor: { userId: string; role?: string },
  ) {
    this.ensureAdmin(actor.role);
    if (!dto.comment?.trim())
      throw new BadRequestException("Reversal comment is required");
