...
- 5 recurring subscriptions (household examples)
- Vendor entries with Clearbit logo URLs, including common subscription vendors (Netflix, Spotify, Disney+, Max, Apple, Google, Microsoft, Adobe, Amazon Prime, YouTube)

Seed behavior is **reset-style idempotent** in development: it clears seed domain data and recreates a deterministic baseline, so repeated runs produce the same resulting dataset.
...
## Recurring distribution metadata

Recurring expense selection state is persisted in `Subscription.distributionRules.userIds` and reused on edit and period generation.
